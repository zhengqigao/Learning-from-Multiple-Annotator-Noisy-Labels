...
from utils.helper import train_network_mbem, seed
...
if __name__ == '__main__':
    ...
    loader_dict = get_loader(file_path, 64, 4, False) # notice that in the ICLR paper, P matrix has size Nsample * K, and it has a certain mapping to the samples. So we cannot let shuffle = True
    ...
    model, acc = train_network_mbem(args.num_epochs, loader_dict, net, device, optimizer, args.iter, args.num_experts, save_model_path='')
    print("Acc MBEM: %f" %acc)
